...
function nvx_enqueue_assets(): void {
	...
	// CSS des couches modulaires (si le fichier existe)
	$css_layers_file = NVX_DIR . '/assets/css/layers/base.css';
	if ( file_exists( $css_layers_file ) ) {
		wp_enqueue_style(
			'nvx-layers-base',
			NVX_URI . '/assets/css/layers/base.css',
			array( 'nvx-style' ),
			NVX_VERSION
		);
	}
